...
impl Grid {
    ...
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
        // Offset can exceed the live grid (wheel-scroll more than one
        // screen, or shrink while scrolled). Saturating math keeps the
        // viewport `rows_len` tall instead of overflowing.
